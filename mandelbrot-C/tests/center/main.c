    ...
    for(int i=0;i<cfg.CmdCount; i++)
    {
        switch (cfg.Commands[i].cmd)
        {
            ...
            case ct_Hold:
                ...
                break;
            case ct_CenterZoom:
                ChangeCenterZoom(&state, cfg.Commands[i].cx, cfg.Commands[i].cy, cfg.Commands[i].zoom, cfg.Commands[i].steps);
                break;
            
            case ct_UnZoom:
                ChangeUnZoom(&state, cfg.Commands[i].zoom, cfg.Commands[i].steps);
                break;
            case ct_CenterRotate:
                ChangeCenterRotate(&state, cfg.Commands[i].cx, cfg.Commands[i].cy, cfg.Commands[i].angle, cfg.Commands[i].steps);
                break;
